...
def step_with_retry(agent, message, max_retries=5):
    delay = 1
    for attempt in range(max_retries):
        try:
            return agent.step(message=message)
        except Exception as e:
            if "rate limit" in str(e).lower():
                logging.warning(f"[⏳] Step rate-limited. Retry {attempt + 1}/{max_retries} in {delay}s...")
                time.sleep(delay)
                delay = min(delay * 2, 16)  # Exponential backoff
            else:
                raise e
    raise Exception("Exceeded retry limit for step() due to rate limiting.")
...
class Avalon(Game):
    ...
    def night_process(self):
        ...
        for player_i in self.alive_players:
            ...
            step_with_retry(self.players[player_i], message=f"reveal phase|" + instruction)
